...
const ProfileModal = () => {
  ...
  const roles = token ? jwtUtils.getJWTData(token)?.roles : null;

  const adminPermission = (roles && Array.isArray(roles) && roles.includes("admin")) ? true : false;
  const riderPermission =  (roles && Array.isArray(roles) && roles.includes("rider")) ? true : false;
  ...
  return (
    <div
      className="min-w-[200px] sm:w-[300px] z-50 bg-white absolute top-full shadow-md shadow-gray-300 p-5 flex flex-col gap-4"
      style={{
        transform: userUtils.isLoggedIn()
          ? "translateX(-40%)"
          : "translateX(-80%)",
      }}
    >
      ...
      {userUtils.isLoggedIn() && (
        <div className="flex flex-col text-gray-600">
          <Link to="/orders" sx={{ textDecoration: "none" }}>
            <div className="cursor-pointer hover:text-pink-600">Orders</div>
          </Link>

          <Link to="/wishlist" sx={{ textDecoration: "none" }}>
            <div className="hover:text-pink-600">Wishlist</div>
          </Link>

          {adminPermission && (
            <Link to="/admin-dashboard" sx={{ textDecoration: "none" }}>
              <div className="hover:text-pink-600">Admin Dashboard</div>
            </Link>
          )}

          {riderPermission && (
            <Link to="/rider-dashboard" sx={{ textDecoration: "none" }}>
              <div className="hover:text-pink-600">Rider Dashboard</div>
            </Link>
          )}
        </div>
      )}
